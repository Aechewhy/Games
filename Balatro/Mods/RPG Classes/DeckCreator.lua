...
local function nextVoucherKey(_from_tag)
    ...
    -- Filter out priority vouchers available in _pool
    local available_priority_vouchers = {}
    for _, voucher in ipairs(priority_vouchers) do
        if table_contains(_pool, voucher) then
            table.insert(available_priority_vouchers, voucher)
        end
    end

    local random_chance = math.random() -- Generate a random number between 0 and 1
    local center

    if #available_priority_vouchers > 0 and random_chance <= 0.33 then
        -- 1/4 chance to select a priority voucher
        center = available_priority_vouchers[math.random(#available_priority_vouchers)]
    else
        -- Otherwise, pick a random voucher from _pool
                        ...
                        func = function()
                            if math.abs(G.shop.T.y - G.shop.VT.y) < 3 then
                                ...
                                if not shop_exists then
                                    ...
                                    if voucherSlots > 1 and G.GAME.current_round.voucher and G.P_CENTERS[G.GAME.current_round.voucher] then
